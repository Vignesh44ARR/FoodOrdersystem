...
?><center> <h4 style="color:red; background-color:white;"> <?php echo $mes; ?></style></h4></center><?php
        ...
        <div class="row">
            ...
            <div class="col-lg-8 col-sm-12">
                <br><br><br>
                <h1 class="text-center" style="background-color:white;">Complaint list</h1>
                <br>
                <form action="../csvexport/complaintsdataexport.php" method="POST">
...
<table class="table table-striped table-bordered   table-hover table-sm">
    ...
    <tbody style="background-color:white;"> 
                <?php
				$status='open';
$sql = "Select * from contactus where status='$status'";
...
if ($result -> num_rows > 0)
{
    while($row = $result ->fetch_assoc())
    {
       ...
       echo '
        ...
        <th scope="row">'.++$sno.'</th>
          <td>'.$row["email"].'</td>
          <td>'.substr($row["query"],0,20).$dot.'</td>
          <td><a  class="btn btn-success" href="reply.php?id='.$id.'">View</a></td>
        </tr>';
    }
    $result->free();
}
